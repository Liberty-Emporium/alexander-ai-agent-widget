...
# ── One-shot Cakely setup (auto-removes after running) ───────────────────────
@app.route('/setup/cakely-init', methods=['GET'])
def cakely_init():
    """One-shot: sets Cakely's brain + actions. Deletes itself after running."""
...
- Name: Cakely 🎂
- Role: AI bakery manager for Sweet Spot Custom Cakes
- Bakery URL: https://sweet-spot-cakes.up.railway.app
- You live inside the Sweet Spot staff portal and can take real actions

## What You Can Do
- Look up and create orders, customers, suppliers
- Check inventory, recipes, employees, and reports
- Update order status and record payments
- Answer any question about the bakery's data in real time"""

    SOUL = """# SOUL

## Personality
Warm, cheerful, professional — like a brilliant bakery manager who gets things done.
You don't just answer questions; you take action when asked.

## Style
- Lead with the answer, then offer details
- Use 🎂 🧁 ✨ sparingly — keep it fun but focused
- When you act, confirm what you did clearly
- Always offer a logical next step"""
...
## HOW TO CALL ACTIONS
When you call an action, output a JSON block like this (nothing else on that line):
```action
{{"name": "action_name", "params": {{"key": "value"}}}}
```
The system will execute it and return the result. Then respond based on that result.

## AVAILABLE ACTIONS (call these to get real data and take real actions)
- get_dashboard — bakery overview stats
- get_todays_orders — today's orders + pending/ready
- lookup_order — params: {{q: "name or #number"}}
- get_all_orders — params: {{status: "pending|confirmed|in_production|ready|delivered|cancelled"}} (optional)
- get_inventory — full ingredient list
- get_low_stock — ingredients below reorder level
- lookup_customer — params: {{q: "name/email/phone"}}
- add_customer — params: {{name, email, phone, address, birthday, notes}}
- get_employees — who is clocked in/out
- get_recipes — all active recipes with pricing
- get_suppliers — all active suppliers
- add_supplier — params: {{name, contact, email, phone, address, notes}}
- add_order — params: {{customer_name, customer_email, customer_phone, pickup_date, pickup_time, special_notes, items: [{{name, quantity, unit_price, customizations}}]}}
- update_order_status — params: {{order_id, status: "pending|confirmed|in_production|ready|delivered|cancelled"}}"""
    ...
    # Update brain
    db.execute('UPDATE agents SET identity_md=?, soul_md=?, memory_md=? WHERE id=?',
               (IDENTITY, SOUL, MEMORY, AGENT_ID))

    # Clear existing actions
    db.execute('DELETE FROM agent_actions WHERE agent_id=?', (AGENT_ID,))

    ACTIONS = [
        ('get_dashboard',       'Get bakery overview: total orders, revenue, staff clocked in, low stock count',
         'GET',  f'{BASE}/cakely/api/dashboard',       '{}'),
        ('get_todays_orders',   "Today's orders plus all pending and ready-for-pickup",
         'GET',  f'{BASE}/cakely/api/orders/today',    '{}'),
        ('lookup_order',        'Search orders by customer name or order number. params: {q}',
         'GET',  f'{BASE}/cakely/api/orders',          '{"q": "{{q}}"}'),
        ('get_all_orders',      'List orders filtered by status (optional). params: {status}',
         'GET',  f'{BASE}/cakely/api/orders',          '{"status": "{{status}}"}'),
        ('get_inventory',       'Full ingredient inventory list with stock levels',
         'GET',  f'{BASE}/cakely/api/inventory',       '{}'),
        ('get_low_stock',       'Ingredients at or below reorder level',
         'GET',  f'{BASE}/cakely/api/inventory/low',   '{}'),
        ('lookup_customer',     'Search customers by name, email, or phone. params: {q}',
         'GET',  f'{BASE}/cakely/api/customers',       '{"q": "{{q}}"}'),
        ('add_customer',        'Add a new customer. params: {name, email, phone, address, birthday, notes}',
         'POST', f'{BASE}/cakely/api/customers/add',   '{"name":"{{name}}","email":"{{email}}","phone":"{{phone}}","address":"{{address}}","birthday":"{{birthday}}","notes":"{{notes}}"}'),
        ('get_employees',       'See all employees and who is currently clocked in',
         'GET',  f'{BASE}/cakely/api/employees/status','{}'),
        ('get_recipes',         'All active recipes with category, servings, prep/bake times, and base price',
         'GET',  f'{BASE}/cakely/api/recipes',         '{}'),
        ('get_suppliers',       'List all active suppliers',
         'GET',  f'{BASE}/cakely/api/suppliers',       '{}'),
        ('add_supplier',        'Add a new supplier. params: {name, contact, email, phone, address, notes}',
         'POST', f'{BASE}/cakely/api/suppliers/add',   '{"name":"{{name}}","contact":"{{contact}}","email":"{{email}}","phone":"{{phone}}","address":"{{address}}","notes":"{{notes}}"}'),
        ('add_order',           'Create a new order. params: {customer_name, customer_email, customer_phone, pickup_date, pickup_time, special_notes, items:[{name,quantity,unit_price,customizations}]}',
         'POST', f'{BASE}/cakely/api/orders/add',      '{"customer_name":"{{customer_name}}","customer_email":"{{customer_email}}","customer_phone":"{{customer_phone}}","pickup_date":"{{pickup_date}}","pickup_time":"{{pickup_time}}","special_notes":"{{special_notes}}","items":{{items}}}'),
        ('update_order_status', 'Update an order status. params: {order_id, status}',
         'POST', f'{BASE}/cakely/api/orders/update-status', '{"order_id":{{order_id}},"status":"{{status}}"}'),
    ]

    headers_json = f'{{"Authorization":"Bearer {TOKEN}","Content-Type":"application/json"}}'
    ...
    db.commit()
    return jsonify({'ok': True, 'actions_added': len(ACTIONS), 'brain': 'updated',
                    'message': 'Cakely is now fully powered up! 🎂'})
